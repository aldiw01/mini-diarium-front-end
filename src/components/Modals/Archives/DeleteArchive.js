...
class DeleteArchive extends Component {
  render() {
    ...
    return (
      <Modal isOpen={_delete} toggle={() => toggleDelete(id)} className={'modal-danger modal-sm'}>
        <ModalHeader toggle={() => toggleDelete(id)}>Delete Dokumen</ModalHeader>
        <ModalBody>
          Do you really want to delete<br />{data.id}?
        </ModalBody>
        <ModalFooter>
...
DeleteArchive.propTypes = propTypes;
